...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=128,
        help='Hidden dim of DeepFM tower fed into gate (larger = more weight on DeepFM side)',
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=64,
        help='Hidden dim of BehaviorEncoder tower fed into gate',
    )
...
def _build_int_array_fields(
    schema: PCVRParquetDataset,
    group: str,
) -> List[Tuple[int, int, int]]:
    """Return list of (fid, vocab_size, dim) for array (dim>1) int features."""
...
def main():
    ...
    user_int_array_fields = _build_int_array_fields(
        train_ds,
        'user',
    )

    item_int_array_fields = _build_int_array_fields(
        train_ds,
        'item',
    )
    ...
    logger.info(
        ...
        f"user_int_array fields: "
        f"{len(user_int_array_fields)} | "
        f"item_int_array fields: "
        f"{len(item_int_array_fields)}"
    )
    ...
    if args.resume_from is not None:
        ...
        deepfm = DeepFM(
            ...
            user_int_array_fields=user_int_array_fields,
            item_int_array_fields=item_int_array_fields,
            dense_dim=dense_dim,
            num_items=num_items,
        ...
        model = GatedFusion(
            ...
            deepfm_hidden_dim=args.deepfm_hidden_dim,
            behavior_hidden_dim=args.behavior_hidden_dim,
            action_num=args.action_num,
        )
    ...
    model_config = {
        ...
        "fusion": {
            ...
            "deepfm_hidden_dim":
                args.deepfm_hidden_dim,

            "behavior_hidden_dim":
                args.behavior_hidden_dim,
        }
    }
