...
def _unpack_deepfm(
    ...
    Keys required by DeepFM:
      ...
      - user_int_array_feature_{fid} for each fid in user_int_array_fids
      - item_int_array_feature_{fid} for each fid in item_int_array_fids
      - item_id
    ...
    for fid in deepfm.user_int_array_fids:
        key = f"user_int_array_feature_{fid}"
        deepfm_batch[key] = batch[key].to(device, non_blocking=True)

    for fid in deepfm.item_int_array_fids:
        key = f"item_int_array_feature_{fid}"
        deepfm_batch[key] = batch[key].to(device, non_blocking=True)
