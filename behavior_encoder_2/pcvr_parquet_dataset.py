...
# fids that are derived from ``timestamp`` at batch-conversion time and have
# no corresponding parquet column; skip them in the parquet-read plans.
_TIMESTAMP_DERIVED_FIDS = {1000, 1001, 1002, 1003}
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        static_fids: Optional[List[Tuple[int, str, int]]] = None,
        static_array_fids: Optional[List[Tuple[int, str, int, int]]] = None,
        static_dense_fids: Optional[List[Tuple[int, str]]] = None,
        static_dense_array_fids: Optional[List[Tuple[int, str, int]]] = None,
    ) -> None:
        """
        Args:
            static_fids:             Int scalar static: [(fid, group, vocab_size), ...].
            static_array_fids:       Int array static:  [(fid, group, vocab_size, max_dim), ...].
            static_dense_fids:       Dense scalar static: [(fid, group), ...].
                                     group is 'user' (reads user_dense_feats_{fid}).
                                     Batch key: ``static_dense`` shape (B, n_dense).
            static_dense_array_fids: Dense array static: [(fid, group, dim), ...].
                                     Batch key: ``static_dense_array_{fid}`` shape (B, dim).
        """
        super().__init__()
        ...
        # ── Static feature config (for StaticEncoder) ──────────────────
        self._static_fids: List[Tuple[int, str, int]] = list(static_fids) if static_fids else []
        self.static_vocab_sizes: List[int] = [vs for _, _, vs in self._static_fids]

        # Int array static
        self._static_array_fids: List[Tuple[int, str, int, int]] = (
            list(static_array_fids) if static_array_fids else []
        )
        self.static_array_configs: List[Tuple[str, int, int]] = [
            (f"static_array_{fid}", vs, dim)
            for fid, _grp, vs, dim in self._static_array_fids
        ]

        # Dense scalar static: [(fid, group), ...]
        self._static_dense_fids: List[Tuple[int, str]] = (
            list(static_dense_fids) if static_dense_fids else []
        )
        # n_dense_scalar for BehaviorEncoder
        self.n_dense_scalar: int = len(self._static_dense_fids)

        # Dense array static: [(fid, group, dim), ...]
        self._static_dense_array_fids: List[Tuple[int, str, int]] = (
            list(static_dense_array_fids) if static_dense_array_fids else []
        )
        # dense_array_configs for BehaviorEncoder: [(name, dim), ...]
        self.dense_array_configs: List[Tuple[str, int]] = [
            (f"static_dense_array_{fid}", dim)
            for fid, _grp, dim in self._static_dense_array_fids
        ]
        ...
        for fid, vs, dim in self._user_int_cols:
            if dim != 1:
                continue
            if fid in _TIMESTAMP_DERIVED_FIDS:  # no parquet column; derived at runtime
                continue
        ...
        # ---- Static feature plan (for StaticEncoder) ----
        # Each entry: (batch_key, fid, vocab_size)
        # batch_key is the key already present in the result dict produced by
        # _convert_batch (either from a parquet column or timestamp-derived).
        #
        # Important: timestamp-derived fids (1000-1003) are NOT in
        # _user_int_plan (they have no parquet column, skipped by
        # _TIMESTAMP_DERIVED_FIDS guard), but _convert_batch does write them
        # as 'user_int_feats_{fid}' AFTER the static_ids assembly would run.
        # We resolve this by:
        #   1. Including 1000-1003 in the key map explicitly.
        #   2. Assembling static_ids AFTER the timestamp block in _convert_batch.
        self._static_plan: List[Tuple[Optional[str], int, int]] = []
        # Scalar int key map: fid → batch-dict key
        _user_int_key = {fid: f'user_int_feats_{fid}'
                         for _, fid, _, _ in self._user_int_plan}
        # Extend with timestamp-derived fids (always present in _convert_batch)
        for fid in _TIMESTAMP_DERIVED_FIDS:
            _user_int_key[fid] = f'user_int_feats_{fid}'
        _item_int_key = {fid: f'item_int_feats_{fid}'
                         for _, fid, _, _ in self._item_int_plan}
        for fid, group, vs in self._static_fids:
            if group == 'user':
                key = _user_int_key.get(fid)
            else:
                key = _item_int_key.get(fid)
            if key is None:
                logging.warning(
                    f"static_fids: fid={fid} group={group} not found in scalar int plans; "
                    "will be filled with zeros in static_ids."
                )
            self._static_plan.append((key, fid, vs))

        # Pre-allocate static buffer
        self._buf_static = np.zeros((batch_size, max(len(self._static_fids), 1)), dtype=np.int64)

        # ---- Array static feature plan (for StaticEncoder array branch) ----
        # Reuses already-produced batch keys: user_int_array_feature_{fid} or
        # item_int_array_feature_{fid}.  We just record those keys and the
        # output key name "static_array_{fid}".
        _user_array_key = {fid: f'user_int_array_feature_{fid}'
                           for _, fid, _, _, _ in self._user_int_array_plan}
        _item_array_key = {fid: f'item_int_array_feature_{fid}'
                           for _, fid, _, _, _ in self._item_int_array_plan}
        # plan entry: (src_key, out_key, fid, max_dim)
        self._static_array_plan: List[Tuple[Optional[str], str, int, int]] = []
        for fid, group, vs, max_dim in self._static_array_fids:
            if group == 'user':
                src_key = _user_array_key.get(fid)
            else:
                src_key = _item_array_key.get(fid)
            if src_key is None:
                logging.warning(
                    f"static_array_fids: fid={fid} group={group} not found in "
                    "int_array plans; 'static_array_{fid}' will be all-zeros."
                )
            out_key = f"static_array_{fid}"
            self._static_array_plan.append((src_key, out_key, fid, max_dim))

        # Pre-allocate array static buffers
        self._buf_static_array: Dict[str, np.ndarray] = {
            out_key: np.zeros((batch_size, max_dim), dtype=np.int64)
            for _, out_key, _, max_dim in self._static_array_plan
        }

        # ---- Dense scalar static plan ----------------------------------------
        # Reuses user_dense_feats_{fid} keys already in result.
        # Only user-side dense features supported (schema has no item_dense).
        # plan entry: (batch_key, fid)
        _user_dense_key = {fid: f'user_dense_feats_{fid}'
                           for _, fid in self._user_dense_plan}
        self._static_dense_plan: List[Tuple[Optional[str], int]] = []
        for fid, group in self._static_dense_fids:
            key = _user_dense_key.get(fid) if group == 'user' else None
            if key is None:
                logging.warning(
                    f"static_dense_fids: fid={fid} group={group} not found in "
                    "dense plans; will be zeros in static_dense."
                )
            self._static_dense_plan.append((key, fid))
        # Pre-allocate dense scalar buffer: (B, n_dense) float32
        self._buf_static_dense = np.zeros(
            (batch_size, max(self.n_dense_scalar, 1)), dtype=np.float32
        )

        # ---- Dense array static plan -----------------------------------------
        # Batch key: static_dense_array_{fid}  shape (B, dim) float32.
        # No existing per-fid key for dense arrays in this dataset; we read the
        # user_dense_feats_{fid} key, which for multi-dim features is NOT produced
        # (dataset only emits dim==1 dense scalars).  For now we emit zeros and
        # expect the user to extend _convert_batch if raw dense arrays are needed.
        self._static_dense_array_plan: List[Tuple[str, int]] = [
            (f"static_dense_array_{fid}", dim)
            for fid, _grp, dim in self._static_dense_array_fids
        ]
        self._buf_static_dense_array: Dict[str, np.ndarray] = {
            out_key: np.zeros((batch_size, dim), dtype=np.float32)
            for out_key, dim in self._static_dense_array_plan
        }
        ...
        logging.info(
            ...
            f"static_int_scalar_fields={len(self._static_fids)}, "
            f"static_int_array_fields={len(self._static_array_fids)}, "
            f"static_dense_scalar_fields={len(self._static_dense_fids)}, "
            f"static_dense_array_fields={len(self._static_dense_array_fids)}")
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        # Appended to _user_int_cols so _build_int_fields() in train.py
        # picks them up automatically without any schema.json changes.
        #   fid=1000  hour         vocab=24  (0-23)
        #   fid=1001  day_of_week  vocab=7   (0=Mon … 6=Sun)
        #   fid=1002  is_weekend   vocab=2   (0=weekday, 1=weekend)
        #   fid=1003  time_slot    vocab=4   (0=night, 1=morning,
        ...
        existing_fids = {fid for fid, _, _ in self._user_int_cols}
        for entry in _TIME_FEATS:
            if entry[0] not in existing_fids:
                self._user_int_cols.append(list(entry))
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        # ── Timestamp-derived user features ──────────────────────────
        # timestamps shape: [B], Unix seconds (int64).
        # Unix epoch 1970-01-01 is a Thursday; +3 shifts it so Mon=0.
        #
        #   user_int_feats_1000  hour        (0-23)
        #   user_int_feats_1001  day_of_week (0=Mon … 6=Sun)
        #   user_int_feats_1002  is_weekend  (0=weekday, 1=weekend)
        #   user_int_feats_1003  time_slot   (0=night 0-5h,
        #                                     1=morning 6-11h,
        #                                     2=afternoon 12-17h,
        #                                     3=evening 18-23h)
        _hour = (timestamps // 3600) % 24
        _dow  = ((timestamps // 86400) + 3) % 7
        result['user_int_feats_1000'] = torch.from_numpy(_hour.astype(np.int64).copy())
        result['user_int_feats_1001'] = torch.from_numpy(_dow.astype(np.int64).copy())
        result['user_int_feats_1002'] = torch.from_numpy(
            (_dow >= 5).astype(np.int64).copy()
        )
        result['user_int_feats_1003'] = torch.from_numpy(
            (_hour // 6).astype(np.int64).copy()
        )

        # ── Static feature tensor (for StaticEncoder) ─────────────────
        # Assembled from already-computed per-fid tensors in result{}.
        # Shape: (B, n_static_fields), dtype int64.
        if self._static_fids:
            buf = self._buf_static[:B]
            for i, (key, fid, vs) in enumerate(self._static_plan):
                if key is not None and key in result:
                    buf[:, i] = result[key].numpy()
                else:
                    buf[:, i] = 0
            result['static_ids'] = torch.from_numpy(buf.copy())

        # ── Array static feature tensors (for StaticEncoder array branch) ─
        # Each array field is already in result as user/item_int_array_feature_{fid}.
        # We create an additional key static_array_{fid} (B, max_dim) that
        # StaticEncoder will read by name.
        for src_key, out_key, fid, max_dim in self._static_array_plan:
            buf = self._buf_static_array[out_key][:B]
            if src_key is not None and src_key in result:
                src = result[src_key]  # (B, src_dim) tensor
                src_dim = src.shape[1]
                copy_len = min(src_dim, max_dim)
                buf[:, :copy_len] = src[:, :copy_len].numpy()
                if max_dim > copy_len:
                    buf[:, copy_len:] = 0
            else:
                buf[:] = 0
            result[out_key] = torch.from_numpy(buf.copy())

        # ── Dense scalar static tensor → 'static_dense' (B, n_dense) float32 ─
        if self._static_dense_fids:
            buf_d = self._buf_static_dense[:B]
            for i, (key, fid) in enumerate(self._static_dense_plan):
                if key is not None and key in result:
                    buf_d[:, i] = result[key].numpy().astype(np.float32)
                else:
                    buf_d[:, i] = 0.0
            result['static_dense'] = torch.from_numpy(buf_d.copy())

        # ── Dense array static tensors → 'static_dense_array_{fid}' (B, dim) ─
        for out_key, dim in self._static_dense_array_plan:
            buf_da = self._buf_static_dense_array[out_key][:B]
            buf_da[:] = 0.0   # currently zero-filled; extend here if raw dense arrays needed
            result[out_key] = torch.from_numpy(buf_da.copy())
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        static_fids=static_fids,
        static_array_fids=static_array_fids,
        static_dense_fids=static_dense_fids,
        static_dense_array_fids=static_dense_array_fids,
    )
